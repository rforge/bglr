...
.onAttach = function(library, pkg)
{
  ...
  assign(".BGLR.ff.home", file.path(library, pkg),
         pos=match("package:BGLR.ff", search()))
  BGLR.ff.version = "1.0.4 (2014-10-29), build 90"
  assign(".BGLR.ff.version", BGLR.ff.version, pos=match("package:BGLR.ff", search()))
  if(interactive())
  {
    packageStartupMessage(paste("Package 'BGLR.ff', ", BGLR.ff.version, ". ",sep=""),appendLF=TRUE)
    packageStartupMessage("Type 'help(BGLR.ff)' for summary information",appendLF=TRUE)
  }
  invisible()
}
        ...
        if (nLT > 0) {
            for (j in 1:nLT) {
                ...
                if (ETA[[j]]$model == "FIXED") {
                  ...
                  ans = .Call("sample_beta_ff",ETA[[j]]$input_ff_filename, n, ETA[[j]]$p, ETA[[j]]$X, ETA[[j]]$x2, ETA[[j]]$b, 
                                             e, varBj, varE, 1e-9)
                  ETA[[j]]$b = ans[[1]]
                  e = ans[[2]]
