...
1) Do we need fseek here?
2) Convert any 8 bit binary number to integer, now only works with 0,1 and 2
*/

SEXP sample_beta_ff(SEXP filename, SEXP n, SEXP pL, SEXP XL, SEXP xL2, SEXP bL, SEXP e, SEXP varBj, SEXP varE, SEXP minAbsBeta)
{
    double *xj;
    double *pXL, *pxL2, *pbL, *pe, *pvarBj;
    double b;
    ...
    pfilename = CHAR(STRING_ELT(filename, 0));

    //PROTECT(filename = AS_CHARACTER(filename));
    //pfilename = R_alloc(strlen(CHAR(STRING_ELT(filename, 0))), sizeof(char));
    //strcpy(pfilename, CHAR(STRING_ELT(filename, 0)));
    ...
    buffer = (char *) R_alloc(rows,sizeof(char));
    ...
    if(input!=NULL)
    {
    	...
    	for(j=0; j<cols;j++)
    	{
          fread(buffer,sizeof(char),rows,input);
          for(i=0; i<rows;i++)
	  {
	...
	error("It was not possible to open the ff binary file %s",pfilename);
     }
