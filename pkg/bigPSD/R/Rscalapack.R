# ====================================================================
#           R-bigPSD version 0.4.x:  bigPSD interface to R
...
# implied warranty.
#
# bigPSD (http://www.aspect-sdm.org/Parallel-R) was funded
...
sla.pdgesv<- function () {
	abc<-.Call("CR_Exec", PACKAGE="bigPSD")
	if (abc == 0) {
...
# Check block size for proper distribution
	MB = sla.checkBlockSize(dimA, MB)
	inputVector <- list(A,as.numeric(NULL),as.integer(NPROWS),as.integer(NPCOLS),as.integer(MB), as.integer(3), as.integer(RFLAG),as.integer(SPAWN))
...
# Check block size for proper distribution
	...
	inputVector <- list(A,as.numeric(NULL),as.integer(NPROWS),as.integer(NPCOLS),as.integer(MB), as.integer(4), as.integer(RFLAG),as.integer(SPAWN))
...
# Check block size for proper distribution
	...
	inputVector <- list(A,as.numeric(NULL),as.integer(NPROWS),as.integer(NPCOLS),as.integer(MB), as.integer(5), as.integer(RFLAG),as.integer(SPAWN))
...
# Check block size for proper distribution
	...
	inputVector <- list(A,as.numeric(NULL),as.integer(NPROWS),as.integer(NPCOLS),as.integer(MB), as.integer(6),as.integer(RFLAG),as.integer(SPAWN))
...
sla.gridInfo <- function () {
	x<-.External("PA_GridInfo", PACKAGE="bigPSD");
...
# Check for illigal values
	...
	inputVector <- list(as.numeric(NULL), as.numeric(NULL), as.integer(NPROWS),
...
sla.gridExit <- function() {
	inputVector <- list(as.numeric(NULL),as.numeric(NULL),as.integer(0),as.integer(0),as.integer(0), as.integer(0), as.integer(1),as.integer(0));
	x <- PA.exec("bigPSD", "pdgesv.R",inputVector);
	rm( ".RscalaGrid", envir=.GlobalEnv);
    print ("bigPSD:Process Grid Released " )  
}
...
sla.tcrossprod <- function( A, NPROWS=0, NPCOLS=0, MB=16, RFLAG=1, SPAWN=1 ) {
# Check for illigal values
...
#Check for 2D matrix
        if( dimAlength != 2)
        {       
                stop("The matrix is not a 2D matrix.\n3D matrices are not yes supported.")
        }
...
# Check block size for proper distribution
	...
	inputVector <- list(A,as.numeric(NULL),as.integer(NPROWS), as.integer(NPCOLS), as.integer(MB), as.integer(8),as.integer(RFLAG),as.integer(SPAWN))
